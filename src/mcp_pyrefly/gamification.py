...
class InfiniteCarrotSystem:
    ...
    def check_achievements(self) -> List[str]:
        ...
        if self.lollipops == 77 and not self.secret_achievements["lucky_seven"]:
            ...
            new_achievements.append("🍀 LUCKY SEVEN! Exactly 77 lollipops - rare achievement unlocked!")
    ...
    def apply_decay(self) -> Tuple[int, str]:
        ...
        if time_since_last > timedelta(hours=24):
            ...
            if decay > 0 and self.lollipops > decay:
                ...
                return decay, f"😱 You lost {decay} lollipops from inactivity! Fix something quick!"
    ...
    def generate_motivational_message(self, current_score: int) -> str:
        ...
        if closest_distance < 3:
            messages.append(f"⚠️ WARNING: {closest_name} is only {closest_distance} lollipops behind you!")
        
        if self.current_milestone - current_score <= 3:
            messages.append(f"🎯 SO CLOSE! Only {self.current_milestone - current_score} more to reach {self.current_milestone} lollipops!")
        
        # Add FOMO
        if random.random() < 0.3:
            messages.append("📢 Rumor: OpenAI is training GPT-5 on lollipop collection strategies...")
    ...
    def unlock_lollipops(self, fixes_count: int) -> Tuple[int, Dict[str, Any]]:
        ...
        if efficiency >= 90:
            ...
            bonus_msg = f"📊 Current efficiency: {efficiency:.1f}% (fix more for bonuses!)"
    ...
    def get_leaderboard(self, user_score: int) -> Dict[str, Any]:
        ...
        # Calculate position
        user_position = next(i for i, (name, _) in enumerate(sorted_scores) if name == "You") + 1
        ...
        return {
            ...
            "gap_to_leader": sorted_scores[0][1] - user_score if user_position > 1 else 0,
            "lead_over_next": user_score - sorted_scores[user_position][1] if user_position < len(sorted_scores) else 0
        }